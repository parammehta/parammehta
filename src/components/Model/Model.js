...
export const Model = ({
  ...
  useEffect(() => {
    ...
    // `renderFrame` is declared below via useCallback; these subscriptions only
    // fire later (after mount), by which point it's fully assigned.
  ...
  // React Compiler isn't enabled for this project; this useCallback memoization
  // works as intended at runtime regardless of compiler analysis.
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      gltf.scene.traverse(async node => {
        if (node.material) {
          // Color management is on by default since three r152, so the hex is
          // converted from sRGB automatically - no manual convertSRGBToLinear.
    ...
    // Stores the loader so the effect below can trigger it once `show` flips.
