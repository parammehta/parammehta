...
export const DisplacementSphere = props => {
  ...
  const { themeId, colorWhite, rgbAccent } = theme;
  // useRef only uses this argument on the first call; re-evaluating Date.now()
  // on later renders is wasteful but not incorrect since the result is discarded.
  ...
  useEffect(() => {
    ...
    // rgbAccent seeds the shader uniform on mount only — the effect below keeps it
    // in sync afterwards. Listing it here would tear down and rebuild the whole
    // WebGL context on every theme change.
