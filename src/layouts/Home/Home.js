...
export const Home = () => {
  ...
  const groupIntuit = useRef();
  const groupRivian = useRef();
  const groupWalmart = useRef();
  const experienceOne = useRef();
  const experienceTwo = useRef();
  const experienceThree = useRef();
  const experienceFour = useRef();
  const experienceFive = useRef();
  const experienceSix = useRef();
  const skills = useRef();

  // DOM node refs are stable for the component's lifetime once attached, so
  // reading `.current` here to check scroll-visibility is safe.
